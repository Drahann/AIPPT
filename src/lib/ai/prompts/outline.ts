...
4. 语义驱动模板匹配（核心原则）：
    - 【数据与指标】：若内容核心在于展现多组连贯的具体数值、对比趋势或业绩占比时，可酌情选用 \`metrics\` (四到八项), \`metrics-rings\` (少量), \`chart-bar\`, \`chart-line\` (时间序列趋势), \`chart-pie\`。如果只是正文中含有零星数字，请依旧使用常规的文字卡片（cards类）以保证排版稳定性。
