...
ap.add_argument('--clean', action='store_true',
                help='only CLEAN parts (source=slide/chrome = real embedded PNG); '
                     'excludes source=vector render-crops that bake in template placeholder text')
ap.add_argument('--source', default='', help='filter by source: slide|chrome|vector')
a = ap.parse_args()
...
def ok(r):
    ...
    if a.theme and a.theme.lower() not in str(r.get('theme_meaning', '')).lower():
        return False
    if a.clean and r.get('source') not in ('slide', 'chrome'):
        return False
...
for r in uniq:
    ...
    paths.append(p)
    src = r.get('source', '?')
    clean = 'CLEAN' if src in ('slide', 'chrome') else 'dirty'
    print(f"{r['function']:15} {str(r.get('family','')):15} {clean:5} {src:6} {str(r.get('theme_meaning',''))[:10]:10} aspect={r.get('aspect')}  {p}  {r.get('desc','')}")
