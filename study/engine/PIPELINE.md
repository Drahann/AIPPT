...
> **操作方法以 `.claude/skills/chuangsai-deck-gen/SKILL.md` 为准**（复刻参考、首过即像、用参考自己的皮、页级冷启动、禁跨页复用 AI 图、装饰按框现场生）。本文是**引擎/schema/坑**的参考，不是流程主文档。
> 一句话：**每页选一张真页，照它复刻——同底图/同配色/同装饰/同布局，只换文字、统一字体**；确定性 Python 渲染器(`render_page`)把文字 shapes + 装饰 extras 扣到 clean_base 上出图。
...
| `deco_skeleton.py` | 抽参考页装饰骨架(function@box + 文字槽真字号 + 密度) | 选页后必跑,继承装饰摆放与起手字号 |
| `deco_check.py` | 校验"只多不少"(装饰+图片数≥模板 + 真资产够多) | S6 必跑,FAIL 回去加真装饰 |
| `deco_stats.py` | 装饰使用 playbook(每页型装饰/页均值+function命中率) + clean/dirty 库存 | 排版前看,对齐模板装饰密度 |
| `reclassify_family.py` | 修 family 错标(改标签+移文件,不重建库) | 改完跑 `corpus_index.py`;family 是手工标会错 |
| `assets_search.py` | 按 function×family 取装饰件路径(`--clean` 只取干净嵌入件) | dirty=render裁块烤死占位文字,默认加 `--clean` |
| `dashscope_t2i.py` | 文生图(默认 `qwen-image-2.0-pro`,按模型名分发同步/异步端点) | qwen-image-2.x 走 multimodal 同步、可图生图 |
...
### 7.1 · 资产工具与约束（配合 SKILL 复刻流程）
- **资产主源 = `corpus/<deck>/`**（每页 json 含每个装饰的 desc/位置/图片路径）；`assets_lib/` 为辅助索引。脱离 json 的裁块无法直接用——不合适=换别套同类或 AI 生，不是手画。
- **`deco_skeleton.py <page_id>`**：抽参考页每个装饰位 function@box + corpus 真图路径 + 文字槽真字号 + 密度。复刻时照它摆装饰、起手字号。
- **`assets_search.py --clean`**：只取真嵌入 PNG(`source∈slide/chrome`)；`source=vector` 是裁块(烤死占位文字)不可用。
- **`deco_check.py <binding> --ref <page_id> [--cards N]`**（安全网）：卡片必有 card_frame 图、结构功能必有真图、自画 rect/line/pill 不计数、同一张图铺不同比例 FAIL(SIZE-FIT)。
- **`reclassify_family.py`**：family 是建库手工 `--family` 参数会错标→改标签+移文件不重建库(改完跑 `corpus_index.py`)。
- **生图**：默认 `qwen-image-2.0-pro`(multimodal 同步端点,可参考图做图生图;`dashscope_t2i.py` 按模型名分发)。`--palette` 传颜色词不传 hex；**每张装饰按其 box 尺寸单独生、同比例才复用、禁跨页复用**。
- **装饰密度参照**(`deco_stats.py`/页)：cover≈9.5 market≈11.6 solution≈13.2 content≈10.9 team≈11.5 financing≈11 roadmap≈12.7。
