...
3. **一切"小装饰"底层都是图片，绝对禁止自画矢量充数**。卡片套 `card_frame` 图、要点坐 `pill/bullet_marker` 背景图、大数字垫 `number_backplate` 图、标题配 `title_flank`、四角 `corner_hud`、主视觉 `motif`、年份/序号/小标签坐**真徽章/胶囊/标签条图**、**箭头/连接线坐真 `connector/arrow` 图**——凡是这类小的强调/装饰件(胶囊、图标、点、箭头、连接线、角标、分隔花、徽章、绶带…)**一律不自画**，**优先复用素材库/参考的 corpus 真图，否则 `gen_deco` 按框现生**。**任何 `pill`/填充 `polygon`/圆角底板 `rect`/自画箭头线 当装饰 = 直接判负**(`deco_check` 的 NO-SELF-DRAW 门)。唯一可用矢量的：纯数据图(`pie/cards/bar`)、**结构性逻辑图**(见 §7)、坐标轴/极细分隔线、整版 scrim。**优先复用参考自己的装饰图**(`deco_skeleton` 的 corpus 真图路径)；不合适就 `gen_deco` 按框现生。

4. **每页冷启动、独立文件夹、零跨页复用、禁偷看别页**。每页一个 `pXX_<名>/_assets/`，里面**只准**放：(a)从**原参考 deck 的 `corpus/<deck>/assets/`** 拷来的真装饰，或 (b)为**本页这些框**新生的 AI 图。**严禁** `cp` 隔壁页 `_assets` 的任何 AI 生图(底图/导航/光晕/分子…)——跨页复用，`page_gate` 的 PROVENANCE 门按字节哈希判负。"同尺寸框可复用同一张"**只在本页内部**成立，不跨页。排到框才按框尺寸现生，禁开工前批量备图。**更禁止打开/阅读/仿写其它页的 `pXX.py` 代码**——每页都从参考 + 本 SKILL **从零设计**自己的版式与代码，不许照抄已有页的写法(那会让全本同质化、且掩盖真实能力)。可用的共享件只有引擎工具与 `deckgen_lib` 的 `Page` 助手。
...
7. **干净底 + 加法合成 + 数据图/逻辑图各按其法**。底图用 `clean_base`(无内容纯场景)，装饰一张张精确摆上(不用 `make_plate` 拍平，会带残留)。
   - **数据图表(饼/柱/折)= 矢量真数字重画**：用 `pie/cards/bar`(`deckgen_lib` 已有) + json 真数据，永不 AI 生数据图、永不照抄模板里的数。
   - **结构性逻辑图(时间轴 / 路线图 / 金字塔 / 漏斗 / 流程 / 循环环 / 矩阵 / 韦恩 / 鱼骨 …)= 重点特化、二选一**：①**矢量自画**(这类整体结构图**允许**自画——它是信息结构本身，不是"小装饰"；用 `line/rect/polygon/pie` 拼，节点上的胶囊/箭头/图标仍走真图)；②或 **AI 生整图**(这类**允许带烤进去的文字/标签**，因为是一张完整示意图)。**禁止**把这种结构图退化成"一堆自画小方块+自画箭头"冒充——要么是干净的矢量结构图、要么是 AI 完整示意图。选页时优先挑**结构与本页逻辑(几段/几层/几步)匹配**的参考。
