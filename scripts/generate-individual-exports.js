...
console.log(`Generating ${files.length} export scripts...`);

files.forEach(file => {
    const slideIndex = file.split('-').pop().replace('.json', '');
    const scriptPath = path.join(SESSION_DIR, `export-slide-${slideIndex}.js`);
...
const SNAPSHOT_PATH = path.join(__dirname, '${file}');
...
console.log("Done! You can now run each script individually or use a batch command.");
